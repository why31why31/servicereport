...
def optimize_image(uploaded_file, max_res=(500, 500)):
    """Mengecilkan ukuran file gambar secara signifikan untuk mencegah hang"""
    ...
    def header(self):
        if self.page_no() == 1:
            if self.logo_img:
                ...
                logo_h = 18 # Ukuran logo proporsional
    ...
    if extra_items:
        ...
        for i, item in enumerate(extra_items):
            if i > 0 and i % 4 == 0:
                ...
                start_y = pdf.get_y() + 5
            col, row = i % 2, (i // 2) % 2
            x_pos, y_pos = margin_left + (col * (cw + gap)), start_y + (row * (rh + 10))
            if y_pos + rh > 280:
                pdf.add_page()
            ...
            if col == 1 or i == len(extra_items)-1: pdf.set_y(y_pos + rh + 5)
