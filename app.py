...
if "authenticated" not in st.session_state:
    ...
    st.markdown("<style>iframe{border:1px solid #ddd !important; border-radius:10px; background-color:white;}</style>", unsafe_allow_width=True)

    client = get_gspread_client()
    
    # PERBAIKAN SELEKTIF: Bersihkan session, tapi sisakan draft lokal agar tidak hilang saat logout tak sengaja
    with st.sidebar:
        st.header("App Menu")
        ...
        if st.button("Logout (Keep Local Draft)", use_container_width=True):
            # Logout aman: menghapus status auth tapi mempertahankan teks draft di form Anda
            del st.session_state["authenticated"]
            st.rerun()
            
        if st.button("⚠️ Clear Form & Hard Reset", use_container_width=True, type="secondary"):
            # Jika ingin membersihkan total seluruh ketikan form kembali ke nol
    ...
    # IMPLEMENTASI OPSI 1 & 2: Form terikat langsung ke widget key untuk mempertahankan draft ketikan secara real-time
        ...
        if st.button(btn_label, use_container_width=True):
            if not g_link:
                ...
                try:
                    ...
                    # Pembersihan aman: Kosongkan session state agar form kembali suci setelah data masuk cloud
