...
def create_pdf(data, s_t, s_c, logo_path, photos):
    ...
    # Grid tata letak PDF tetap dibuat efisien
    fields = [
    ...
    pdf.set_font("helvetica", '', 10); pdf.multi_cell(0, 6, data['pr'])
    ...
    pdf.ln(10); curr_y = pdf.get_y(); pdf.set_font("helvetica", 'B', 10)
...
if st.button("🚀 GENERATE PDF REPORT", type="primary", use_container_width=True):
    if not cb:
        ...
        # --- 1. URUTAN DATA SPREADSHEET DISESUAIKAN ---
        # Date, Day, Customer, Machine, Type, Serial No, Problem, Action, Technician, Status
    ...
    if st.button("💾 SAVE TO SPREADSHEET & RESET", use_container_width=True):
        if not g_link:
            ...
            try:
                ...
                # --- 2. OTOMATIS SORT BERDASARKAN TANGGAL (KOLOM A) ---
                # Mengurutkan range A2 sampai K (asumsi kolom terakhir K) secara Ascending (A ke Z)
                sheet.sort((1, 'asc'), range='A2:K2000')
                
                st.success("Data Saved & Sorted by Date!")
