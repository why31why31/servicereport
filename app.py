...
def create_pdf(data, sig_t=None, sig_c=None, logo=None, logo_w=30, extra_items=None):
    ...
    if extra_items:
        ...
        for item in valid_items:
            ...
            if total_item_h > max_row_height: max_row_height = total_item_h
            current_x += img_w + 10
            pdf.set_y(start_y)
...
st.set_page_config(page_title="Service Report System", layout="wide") # Menggunakan layout wide agar preview lega
...
# BAGIAN PREVIEW (Hanya muncul jika sudah klik Simpan)
if st.session_state.get('last_data'):
    st.write("---")
    st.subheader("🔍 Preview Laporan")
    
    # Menampilkan Preview di Container agar rapi
    with st.container(border=True):
        st.info(f"**Technician:** {st.session_state['last_data']['Completed By']} | **Customer:** {st.session_state['last_data']['Customer']}")
        st.write(f"**Machine:** {st.session_state['last_data']['Machine']} | **Serial No:** {st.session_state['last_data']['Serial No']}")
        
        col_p1, col_p2 = st.columns(2)
        with col_p1:
            st.warning("**Problem:**")
            st.write(st.session_state['last_data']['Problem'])
        with col_p2:
            st.success("**Follow Up:**")
            st.write(st.session_state['last_data']['Follow Up'])
            
        # Preview Foto
        st.write("**Dokumentasi Foto:**")
        cp1, cp2 = st.columns(2)
        items = st.session_state['extra_items']
        if items[0]['file']: 
            with cp1: 
                st.image(items[0]['file'], caption=items[0]['caption'], use_container_width=True)
        if items[1]['file']: 
            with cp2: 
                st.image(items[1]['file'], caption=items[1]['caption'], use_container_width=True)

    # TOMBOL DOWNLOAD PDF (Hanya muncul setelah preview ok)
    ...
    st.download_button(
        label="✅ Data Sudah Sesuai? Download PDF Sekarang",
        data=pdf_bytes,
